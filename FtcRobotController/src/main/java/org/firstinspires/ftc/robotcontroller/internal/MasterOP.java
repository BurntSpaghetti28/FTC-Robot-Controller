...
@TeleOp(name="Main Tele0P")
public class MasterOP extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            if (Robot.SetPower) {
                fl.setPower(Robot.FL);
                fr.setPower(Robot.FR);
                rl.setPower(Robot.RL);
                rr.setPower(Robot.RR);
            }
            else if (activated(gamepad1.left_stick_x) || activated(gamepad1.left_stick_y) || activated(gamepad1.right_stick_x)) {
            ...
            if (Robot.SetElevatorPower) {
                el.setPower(-Robot.ElevatorPower);
            }
            else if (gamepad1.y && !tsmax.isPressed()) {
                el.setPower(-1);
            }
